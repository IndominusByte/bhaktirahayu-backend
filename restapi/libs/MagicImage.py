...
class MagicImage:
    ...
    @classmethod
    def save_image_to_storage(cls, **kwargs) -> str:
        """
        Save an image only
        :param file: File-like object
        :return: Filename
        """
        file = kwargs['file']
        path_upload = kwargs['path_upload']
        if 'dir_name' in kwargs: dir_name = kwargs['dir_name']
        ...
        try:
            with Image.open(file) as img:
                ...
                if 'dir_name' in kwargs:
                    # create directory if file isn't exists
                    path = os.path.join(cls.base_dir,path_upload,dir_name)
                    if not os.path.exists(path):
                        os.mkdir(path)
                    img.save(os.path.join(path,filename))
                else:
                    img.save(os.path.join(cls.base_dir,path_upload,filename))
    ...
    @classmethod
    def convert_image_as_base64(cls,path_file: str) -> str:
        path = os.path.join(os.path.join(os.path.dirname(__file__),'../'),path_file or ' ')
